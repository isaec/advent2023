...
fn parse(input: &str) -> Result<(Vec<JoinedNumber>, Vec<(usize, usize)>)> {
    ...
    let symbol_regex = fancy_regex::Regex::new(r"\*").pretty_msg("regex")?;
...
pub fn part2(input: &str) -> Result<u64> {
    let (numbers, stars) = parse(input)?;
    let mut star_adjacency = vec![vec![]; stars.len()];
    numbers
        .iter()
        .for_each(|n| {
            let (x1, x2) = n.x_range;
            let star = stars
                .iter()
                .enumerate()
                .find(|(_, (x, y))| // check if the symbol is adjacent to the number
                    // vertical adjacent
            ...
            if let Some((i, _)) = star {
                star_adjacency[i].push(n.n);
            }
        });

   Ok( star_adjacency.iter().filter(|s| s.len() == 2).map(|s| s.iter().product::<u64>()).sum::<u64>())
}
...
"#};
        dbg!(&input);
        assert_eq!(part2(input).expect("part2 should return Ok"), 467835);
    }
